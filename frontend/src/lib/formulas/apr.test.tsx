...
import { calcAprByInterestEarned, calcAprInterest, calcAverageApr } from "./apr";
...
describe("APR formulas", () => {
  ...
  describe("calcAprInterest", () => {
    it("returns 0n when apr is 0n or amount is 0n", () => {
      expect(calcAprInterest(0n, 1000n, { aprScale: BPS })).toBe(0n);
      expect(calcAprInterest(1000n, 0n, { aprScale: BPS })).toBe(0n);
    });
  ...
  describe("calcAprByInterestEarned", () => {
    it("returns 0n when interest is 0n or amount is 0n", () => {
      expect(calcAprByInterestEarned(0n, 1000n, { aprScale: BPS })).toBe(0n);
      expect(calcAprByInterestEarned(1000n, 0n, { aprScale: BPS })).toBe(0n);
    });
