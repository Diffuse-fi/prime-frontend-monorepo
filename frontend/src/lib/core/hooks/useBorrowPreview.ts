...
const WAD = 10n ** 18n;
const LTV_WAD = 950000000000000000n; // const 95%
    ...
    queryFn: async ({ signal }) => {
      ...
      if (selected.collateralType === 1) {
        ...
        const collateralInBase =
          (selected.collateralAmount * 1_000_000n * 10n ** BigInt(selected.assetDecimals)) /
      ...
      if (selected.collateralType === 1) {
        const collateralInBase = depositPriceWad === 0n
          ? 0n
